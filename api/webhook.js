  ...
  try {
    ...
    if (body.callback_query) {
      ...
      if (cb.data === 'verify') {
        ...
        if (channels.length === 0) {
          ...
          await sendMessage(chatId, `🏠 <b>Main Menu</b>`, mainMenuKeyboard());
    ...
    if (text.startsWith('/start')) {
      ...
      if (existing) {
        ...
        if (channels.length === 0) {
          ...
          await sendMessage(chatId,
            `👋 <b>Welcome, ${firstName}!</b>\n\nPlease join our channels first then press ✅ <b>Verify</b>.`,
            forceJoinKeyboard(channels)
          );
        }
      }
    ...
    if (isAdmin) {
      ...
      if (text.startsWith('/addcontact ')) {
        ...
        if (parts.length < 4) {
          await sendMessage(chatId, `❌ Wrong format.\n<code>/addcontact Name | Phone | Description | Points</code>`, adminMenuKeyboard());
        } else {
          await supabase.from('contacts').insert({
            name: parts[0], phone: parts[1], description: parts[2],
            points_cost: parseInt(parts[3]) || 3, is_available: true,
          });
          await sendMessage(chatId, `✅ Contact <b>${parts[0]}</b> added!`, adminMenuKeyboard());
        }
        return res.status(200).json({ ok: true });
      }

      if (text === '❌ Delete Contact') {
        const { data: contacts } = await supabase.from('contacts').select('*').eq('is_available', true);
        if (!contacts || contacts.length === 0) {
          ...
          contacts.forEach(c => { list += `ID: <b>${c.id}</b> — ${c.name} (${c.points_cost} pts)\n`; });
      ...
      if (text.startsWith('/createcode ')) {
        ...
        if (parts.length < 3) {
          ...
          await supabase.from('redeem_codes').insert({
            code: parts[0].toUpperCase(), points: parseInt(parts[1]) || 10,
            max_uses: parseInt(parts[2]) || 1, used_count: 0,
          });
      ...
      if (text === '📋 List Codes') {
        ...
        if (!codes || codes.length === 0) {
          ...
          codes.forEach(c => { list += `🎟 <b>${c.code}</b> — ${c.points} pts | ${c.used_count}/${c.max_uses} used\n`; });
          await sendMessage(chatId, list, adminMenuKeyboard());
        }
        return res.status(200).json({ ok: true });
      }
      ...
      if (text === '👥 All Users') {
        ...
        if (!users || users.length === 0) {
          ...
          users.forEach((u, i) => { list += `${i + 1}. @${u.username} — <b>${u.points} pts</b> — ID: <code>${u.telegram_id}</code>\n`; });
          await sendMessage(chatId, list, adminMenuKeyboard());
        }
        return res.status(200).json({ ok: true });
      }
    ...
    if (text === '🔷 IVS Panel') {
      ...
      if (!codeData) {
        ...
        if (alreadyUsed) {
          await sendMessage(chatId, `❌ Already used.`, mainMenuKeyboard());
        } else {
      ...
      const { data: contact } = await supabase.from('contacts').select('*').eq('id', contactId).eq('is_available', true).single();
      if (!contact) {
        await sendMessage(chatId, `❌ Contact not found.`, mainMenuKeyboard());
      } else {
        const { data: alreadyBought } = await supabase.from('user_contacts').select('*').eq('telegram_id', userId).eq('contact_id', contactId).single();
        if (alreadyBought) {
          await sendMessage(chatId, `✅ Already purchased!\n\n📞 <b>${contact.name}</b>: <code>${contact.phone}</code>`, mainMenuKeyboard());
        } else if (userData.points < contact.points_cost) {
          await sendMessage(chatId, `❌ Need <b>${contact.points_cost} pts</b>, you have <b>${userData.points} pts</b>.`, mainMenuKeyboard());
        } else {
